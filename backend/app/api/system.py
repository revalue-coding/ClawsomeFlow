...
class NotifyWebhookView(_CamelModel):
    ...
    #: Resolved format the next notification will actually use (null when no
    #: URL is configured). Lets the UI show "recognized as Feishu".
...
class NotifyWebhookPayload(_CamelModel):
    url: str | None = Field(
        default=None,
        description="Webhook URL to POST run events to; empty/null clears.",
    )
    format: str | None = Field(
        default=None,
        description="Outgoing message format; empty/null/'auto' = detect by URL.",
    )
...
def _validated_webhook_format(raw: str | None) -> str | None:
    ...
    if fmt not in WEBHOOK_FORMATS:
        raise ApiError(
            ...
            f"Unknown webhook format {fmt!r}; allowed: auto, "
            + ", ".join(WEBHOOK_FORMATS),
            status_code=422,
        )
    return fmt


def _notify_webhook_view(url: str | None, fmt: str | None) -> NotifyWebhookView:
    from app.services.run_notify import resolve_webhook_format

    return NotifyWebhookView(
        url=url,
        format=fmt,
        effective_format=resolve_webhook_format(url, fmt) if url else None,
    )
...
def get_notify_webhook(_user: UserDep = "") -> NotifyWebhookView:
    ...
    cfg = load_config()
    return _notify_webhook_view(cfg.notify_webhook_url, cfg.notify_webhook_format)
...
def set_notify_webhook(
    ...
    """Set (or clear, with an empty url) the run webhook URL + format."""
    from app import config as cfg_mod

    url = _validated_webhook_url(payload.url)
    fmt = _validated_webhook_format(payload.format) if url else None
    cfg = cfg_mod.load_config().model_copy(
        update={"notify_webhook_url": url, "notify_webhook_format": fmt},
    )
    cfg_mod.save_config(cfg)
    logger.info("notify_webhook_updated", enabled=bool(url), format=fmt or "auto")
    return _notify_webhook_view(url, fmt)
...
async def test_notify_webhook(_user: UserDep = "") -> NotifyWebhookTestResponse:
    ...
    ok, detail = await asyncio.to_thread(
        lambda: post_webhook(url, payload, fmt=cfg.notify_webhook_format),
    )
