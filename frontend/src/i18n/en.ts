...
const en = {
  ...
  directoryBrowser: {
    ...
    viewTitle: "Browse a directory on the server",
    hint:
    ...
    viewHint:
      "Browsing the ClawsomeFlow server's filesystem — the same filesystem agents run in. " +
      "SSH / remote sessions cannot open the host file manager, so the folder is shown here.",
    pathPlaceholder: "/absolute/path/on/the/server",
    go: "Go",
    up: "Up one level",
    empty: "No subdirectories",
    emptyEntries: "No entries to show",
    truncated: "Too many subdirectories — only the first 500 are shown. Type a deeper path above.",
    truncatedEntries: "Too many entries — only the first 500 are shown. Type a deeper path above.",
    showHidden: "Show hidden directories",
    showHiddenEntries: "Show hidden items",
  ...
  shell: {
    ...
    updateDone: "Upgrade complete. Reloading…",
    updateFailed:
  ...
  flowNotify: {
    ...
    configuredTitle: "{{count}} notification channel(s) configured",
    title: "Notifications · {{name}}",
    hint:
      "Get pinged when a run of this Flow finishes (completed / failed / aborted) or pauses waiting for you (manual checkpoint / merge review / complaint window). The message includes the leader's final report — or, at a manual checkpoint, that step's output. Applies to manual and scheduled runs alike.",
    steps:
      "How to set up: 1) In your chat app, add a custom/incoming bot to the target group and copy its webhook URL. 2) Add it as a channel below (format stays on Auto). 3) Test the row, then Save. No relay server or code needed. You can add multiple channels.",
    channelsEmpty: "No channels yet. Add one to enable notifications for this Flow.",
    ...
    save: "Save",
    saving: "Saving…",
    saved: "Saved. Runs of this Flow will now be notified.",
    cleared: "Cleared. Notifications for this Flow are disabled.",
    test: "Send test",
    testing: "Sending…",
  ...
  flowEditor: {
    ...
    remoteCallInfoCopy: "Copy remote call info",
    remoteCallInfoTitle: "Remote call info",
    remoteCallInfoCopied:
      "Remote call info copied to clipboard. Paste it into the origin Flow's \"Remote ClawsomeFlow\" execution node (\"Remote Flow call info\" box); saving the node registers it automatically.",
    ...
    easyModeNotice:
      "In easy mode, agents automatically bring all their results into your project (including when multiple nodes run in parallel) — no per-result confirmation needed; once everything finishes it still enters the user complaint phase so you can give consolidated feedback. Human checkpoints in the flow are still reviewed by you. A timed run instead finishes directly and skips the complaint phase.",
    easyModeAck: "Got it",
    devMode: "Developer mode",
    devModeSub: "Per-node auto-merge control; complaint phase still runs",
    devModeNotice:
      "Developer mode is optimized for software developers, and lets you customize whether each execution node auto-merges changes into the baseline branch. During execution, each agent creates its own unique worktree (an independent branch created from the baseline branch). If you plan to submit PRs directly from worktrees, or need more flexible collaboration control, we recommend disabling agent auto-merge when orchestrating nodes. Agents with auto-merge disabled keep their worktree after the run ends and appear in the \"Pending PRs\" module on the Run detail page (view changes / open a PR in one click / discard the worktree); all other worktrees are deleted automatically after the run ends.",
    devModeAck: "Got it",
    flowName: "Flow name",
    name: "Flow name",
    ...
    leaderHint:
      "The leader aggregates and synthesizes all node outputs, then reports the final outcome back to you.",
    ...
    tasks: "Execution node orchestration",
    tasksHint: "The leader must and only owns the summary node.",
    addTask: "+ Add execution node",
    mergeFlows: "Merge existing Flow",
    mergeModal: {
      title: "Merge existing Flow",
      hint: "Select Flows to merge. All non-summary nodes will be imported, and parameter fields are merged with de-duplication.",
      ...
      conflictDetail:
        "\"{{flow}}\" has node \"{{task}}\" owned by the current leader {{agent}}. Re-assign that node in the source Flow first.",
      partialFailure: "{{count}} Flow(s) failed to load: {{names}}",
      mergedSummary:
        "Merged {{flowCount}} Flow(s); imported {{taskCount}} node(s).",
      preSpecSectionHeader: "### Parameter fields (from {{name}})",
    },
    taskFieldRequired: "Please fill in all required fields before saving.",
    pickLeaderFirst: "Pick a leader at the top before adding execution nodes.",
    mergePickLeaderFirst: "Pick a leader at the top before merging Flows.",
    leaderInUseByTask:
      "Agent \"{{name}}\" is already an owner of one of the nodes below and cannot also be the leader. Re-assign that node first, then switch the leader.",
    ...
    repoIssue: {
      ...
      description: "Node owner \"{{agentId}}\" has an unavailable repository path.",
    ...
    taskRepoCheck: {
      ...
      confirmExistingAgentRepoBranchChange:
        "Agent \"{{agentId}}\" binding (platform / repository / base branch) is about to change. Inside one Flow, nodes sharing the same agent name must share one binding.\n\nContinue and sync this change to all nodes owned by \"{{agentId}}\"?",
    },
    taskBranchCheck: {
    ...
    emptyTasks: "No execution nodes yet. Click \"Add execution node\" at the top right to start authoring.",
    summaryTaskDefaultSubject: "Summary",
    summaryTaskBadge: "Summary node",
    summaryTaskLocked:
      "Auto-generated for the selected leader and cannot be deleted. Executors of its upstream nodes report their work and share their own worktree directories and branches with the leader for further synthesis.",
    summaryNoDepsWarning:
      "Summary currently has no dependencies; pick the upstream nodes to review and report on in \"Depends on\".",
    graphTitle: "Node dependency graph",
    graphLegendTask: "Execution node",
    graphLegendRoot: "Root node",
    graphLegendCheckpoint: "Checkpoint node",
    graphLegendExternal: "External execution node",
    graphLegendSummary: "Summary",
    aiDecompose: "AI Decompose into Nodes (optional)",
    save: "Save Flow",
    saving: "Saving…",
    ...
    deleteFlowOk: "Delete",
    deleting: "Deleting…",
    deleteError: "Failed to delete Flow: {{message}}",
    saveWarnings: {
      openclawRuntimeNotRunning:
        "Flow was saved, but no running OpenClaw service was detected. Nodes assigned to OpenClaw agents may fail until the runtime service starts.",
      generic: "Flow was saved with runtime warning: {{message}}",
    },
    leaderHardConstraint: "Leader must own exactly one node with is_leader_summary=true and no other worker node.",
    rowOwner: "Agent",
    rowDependsOn: "Depends on",
    rowNoDeps: "no deps",
    rowUntitled: "(untitled node)",
    details: "Details",
    edit: "Edit",
    delete: "Delete",
    removeTaskConfirm: "Delete execution node \"{{name}}\"?",
    newTaskModalTitle: "New execution node",
    editTaskModalTitle: "Edit execution node",
    detailTaskModalTitle: "Execution node details",
    modalSave: "Save",
    modalCancel: "Cancel",
    validation: {
      ...
      pickOneSummary: "Please pick a leader at the top — the summary node is generated automatically.",
      onlyOneSummary: "Only one node may be the leader summary.",
      taskIdRequired: "Node id is required.",
      taskIdPattern: "Node id must be [A-Za-z0-9_-]+.",
      duplicateTaskId: "Duplicate node id {{taskId}}.",
      ...
      ownerRepoBranchMismatch:
        "Agent \"{{agentId}}\" has inconsistent workspace repo/target branch across nodes. Keep one repo+branch per Agent, or create a new Agent.",
      ...
      leaderCannotOwnNonSummary: "Leader cannot own a non-summary node.",
      summaryNeedsDependency:
        "The summary node must depend on at least one upstream node to review and report on.",
      cycleDetected: "Node dependencies cannot contain cycles: {{cyclePath}}",
      ...
      externalRemoteParamsRequiredWhenRoot:
        "\"{{subject}}\" is a root node with no upstream dependencies — fill every remote Flow param field (no upstream can supply them).",
    ...
    taskFields: {
      ...
      descriptionPlaceholder: "All work the executor must complete when receiving this node.",
      paramFieldsHintPrefix: "Reference Flow parameter fields:",
      outputSummary: "Output summary requirement",
      outputSummaryHint: "(optional; the summary will be sent to downstream dependent nodes)",
      ...
      ownerKindPi: "Pi Coding Agent",
      ownerKindQoder: "Qoder CLI",
      ownerKindCodebuddy: "CodeBuddy Code",
      ownerKindOpenclaw: "OpenClaw",
      ownerKindHermes: "Hermes",
      ownerKindExternal: "External execution",
      externalNamePlaceholder: "Give this node agent a name",
      externalChannel: "Channel *",
      externalChannelPlaceholder: "— pick a channel —",
      externalChannelHuman: "Human",
      externalChannelWebhook: "Generic interface",
      ...
      externalDispatchCommandHint:
        ...
        + "only delivers the payload — a non-zero exit fails the dispatch and "
        + "pauses the run.",
      ...
      externalHumanHint: "A person completes this node and returns the result on the Run page. On dispatch, the node sheet (description, upstream inputs, output requirement) is pushed to the Flow's notify channels (Flow list → \"Notify\" button) to alert the assignee.",
      ...
      externalRemoteHintWithParams:
        "Delegates to a Flow on a remote ClawsomeFlow; its final report becomes this node’s output. The remote Flow declares param fields: upstream nodes will fill them intelligently; you may also enter known values yourself (they override upstream reports).",
      externalRemoteHintParamsNoUpstream:
        "Delegates to a Flow on a remote ClawsomeFlow; its final report becomes this node’s output. This node has no upstream dependencies — remote param values come only from the fields below.",
      ...
      externalInputsNoUpstreamHint:
        "This node has no upstream dependencies; the remote Flow uses only the values you enter here.",
      ...
      newAgentNamePlaceholder: "Type a name to create one, or pick an existing agent",
      leaderNewAgentPlaceholder: "Type a name to create one, or pick an existing agent",
      ...
      openDirButton: "Open",
      openDirFailed: "Failed to open directory: {{message}}",
      dependsOn: "Depends on (multi)",
      dependsOnHint: "This node only runs once every listed node is completed.",
      isLeaderSummary: "Leader summary node",
      isLeaderSummaryHint: "When checked, this is the leader's single owned summary node.",
      timeoutSeconds: "Timeout (seconds)",
      timeoutHint: "Default 14400 (240 min)",
      requiresHumanCheckpoint: "Require post-task checkpoint",
      requiresHumanCheckpointHint:
        "After this node completes, execution pauses for your review. Downstream nodes dispatch only after approval.",
      ...
      autoMergeOpenclawLocked: "OpenClaw nodes always auto-merge and can't be changed",
      autoMergeSyncedNotice: "Synced merge status for this agent's other nodes",
      submitPrShort: "Submit PR",
      submitPrHint: "After the node completes, the backend pushes the branch and opens a PR to the baseline branch. Independent of auto-merge: with both on, the node merges into the local baseline AND a review PR is opened against the remote (nothing to PR is skipped, not a failure). Per-node setting (not synced across this agent's other nodes).",
      remove: "Remove execution node",
    },
    decompose: {
      title: "AI Decompose Flow Goal",
      hint: "The selected leader agent decomposes the flow goal into a DAG of suggested execution nodes; you can edit after filling results back in.",
      goalLabel: "Flow goal *",
      ...
      result: "Suggested node structure",
      resultCounts: "{{taskCount}} node(s), {{agentCount}} agent(s)",
      apply: "Fill back into node list (overwrites current nodes)",
  ...
  runList: {
    ...
    delegatedTag: "Remote delegated",
    delegatedTagTitle:
      "Delegated by a remote ClawsomeFlow (credential: {{pairToken}}, origin run: {{sourceRun}}). Runs unattended, so human phases are skipped.",
  ...
  scheduledFlows: {
    title: "Timed Flows",
    description:
      "Timed runs are unattended: by default the agent merges all changes and the run finishes directly without a user complaint phase. If the Flow has developer mode on, each node follows its own auto-merge setting and the run still finishes directly. Human checkpoints inserted in the flow are ignored.",
  ...
  runDetail: {
    ...
    flowLabel: "Flow",
    teamLabel: "Team",
    abort: "Terminate flow",
    abortConfirm: "Terminate this flow? This is irreversible — the team and all worktrees are cleaned up and changes are rolled back. Use \"Pause\" instead if you may want to continue later.",
    ...
    delegatedNotice:
      "This run was delegated by a remote ClawsomeFlow and automatically skips human checkpoints, merge review, and the complaint phase. Pairing credential: {{pairToken}}; origin run: {{sourceRun}}; origin node: {{sourceTask}}.",
    ...
    pauseFailureNodeId: "Node ID: {{taskId}}",
    pauseFailureMessageLabel: "Failure message",
    pauseFailureSignal: {
      leader_inbox_failed: "agent reported FAILED",
      worker_reported: "agent marked failed",
      timeout: "timeout",
      dispatch_failed: "dispatch failed",
    },
    pauseFailureSyntheticTimeout: "Node timed out{{detail}}",
    pauseFailureSyntheticFailed: "FAILED: {{taskId}}{{detail}}",
    pauseFailureSyntheticDispatchFailed:
      "Handing work to this node failed{{detail}}",
    ...
    pendingMerges: "Review results",
    pendingMergeHint:
      "Decide for each agent whether to keep its work: click \"Accept changes\" to bring it into your project, or \"Dismiss\" to drop it.",
    branchLabel: "Branch",
    leaderSuggestion: "Leader Suggestion",
    merge: "Accept changes",
    mergeTarget: "Destination",
    viewDiff: "View changes",
    diffModalTitle: "Changes — {{agent}}",
    diffEmpty: "This agent produced no changes.",
    diffTruncated:
      "The changes are too large to show in full (you can inspect the agent's working folder for everything).",
    diffUncommitted: "Unsaved scratch changes (will NOT be brought into your project)",
    diffBranchAhead: "{{count}} commit(s) will be applied",
    diffBaseAhead:
      "Your project gained {{count}} new commit(s) after this agent started working (not shown below). Accepting will layer the agent's work on top; conflicts may need reconciling.",
    ...
    sessionStartFailed:
      "Agent session failed to start (agent {{agent}}, node {{task}}, phase {{phase}}):\n{{error}}",
    boardTitle: "Execution Node Board",
    boardHint: "",
    boardTabList: "List",
    boardTabTerminal: "Terminal",
    boardSplitHandle: "Drag to resize the node list and graph panels",
    replay: {
      button: "Replay",
      exit: "Exit replay",
      play: "Play",
    ...
    boardEmpty: "No node has been dispatched yet. The graph appears once execution starts.",
    boardEdgeInboxTitle: "Inbox message along this dependency",
    boardEdgeInboxEmpty: "(No inbox message yet)",
    boardTaskListTitle: "Executed / Running Nodes",
    boardNodeRunning: "Running",
    boardNodePaused: "Paused",
    boardNodeDone: "Done",
    ...
    terminal: {
      loading: "Loading node terminal output…",
      paneLoading: "Loading terminal output…",
      empty: "No node terminals available.",
      taskListTitle: "Node list",
      paneTitle: "tmux pane",
      ownerLabel: "Owner",
    ...
    leaderReplyFootnote:
      "All outputs and document updates are brought into your project automatically after the task flow completes (see \"Changes from this run\" below), unless you manually discard changes or abort the run.",
    runDiffTitle: "Changes from this run",
    runDiffHint:
      "What this run actually brought into your project. View each agent's contribution, or undo it with one click.",
    runDiffEmpty: "This run brought no changes into your project.",
    ...
    runDiffTruncated:
      "The changes are too large to show in full (you can inspect the project repository for everything).",
    runDiffRevert: "Revert merge",
    runDiffRevertPlain: "Revert changes",
    runDiffReverting: "Reverting…",
    runDiffRevertConfirmTitle: "Revert merge",
    runDiffRevertConfirmTitlePlain: "Revert changes",
    ...
    runDiffRevertConfirmBodyPlain:
      "This will undo {{agent}}'s changes brought into your project this run (by adding inverse commits — no forced edits, no history rewrite). If it can't be done cleanly, nothing is changed and the reason is shown. Revert?",
    ...
    failedAutoMergeHint:
      "These agents were expected to merge into the baseline at node end, but no effective merge was detected. Worktrees are preserved: view changes, merge into the baseline, or discard the worktree (changes stay out of the baseline).",
    ...
    pendingPrDiscardConfirmBody:
      "This deletes {{agent}}'s worktree; its unmerged changes will be lost permanently. Discard?",
    pendingPrDiscardConfirmOk: "Discard",
    pendingPrDiscardFailed: "Discard failed: {{reason}}",
    pendingPrDiffModalTitle: "All changes by {{agent}}",
    eventTypeColumn: "Type",
    eventTimestampColumn: "Time",
    eventAgentColumn: "Agent",
    eventTaskColumn: "Node",
    eventPayloadColumn: "Payload",
    statusBanner: {
      running: "Run is in flight",
      awaitingCheckpoint: "Awaiting your checkpoint decisions",
      awaiting: "Awaiting your decision on each agent's results",
    ...
    complaint: {
      ...
      submittedNotice: "Your complaint has been forwarded to the relevant agents. Each agent will update its outputs and improvements into its own working folder.",
    ...
    external: {
      title: "External execution nodes",
      hint: "Execution nodes assigned to external executors (human / integrated system / remote ClawsomeFlow). The run continues once the result is submitted.",
      itemTitle: "External node - {{channel}}",
      channelHuman: "Human",
      channelWebhook: "Generic interface",
      channelRemoteCsflow: "Remote ClawsomeFlow",
      assignee: "Assignee",
      nodeLabel: "External node",
      showTaskSheet: "Show node sheet",
      ...
      attachmentsCardHint: "Files submitted with external-node receipts; stored in this machine's run directory.",
      ...
      failReasonSubmit: "Confirm report failure",
      submitSuccess: "Submit as completed",
      submitFailed: "Report failure",
      redispatch: "Re-dispatch",
      redispatchConfirm: "Re-dispatching will invalidate the previous dispatch.",
    ...
    checkpoint: {
      ...
      hint: "",
      targetTask: "Blocked downstream node",
      targetOwner: "Blocked owner",
      itemTask: "Current checkpoint node",
      itemOwner: "Owner",
      ...
      rerunModalTitle: "Rerun current checkpoint node",
      rerunFeedbackLabel: "Feedback to current node executor",
  ...
  hermes: {
    ...
    openWorkdir: "Open working directory",
    openWorkdirFailed: "Failed to open working directory: {{message}}",
    workdirSave: "Save",
  ...
  statusLabel: {
    ...
    compiling: "Compiling",
    running: "Running",
    awaiting_external: "Awaiting external results",
