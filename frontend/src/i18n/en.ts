...
const en = {
  ...
  flowEditor: {
    ...
    validation: {
      ...
      externalRemoteFieldsRequired: "\"{{subject}}\": Remote ClawsomeFlow requires base URL, Flow ID and pairing credential name.",
    ...
    taskFields: {
      ...
      externalWebhookHint: "On dispatch, ClawsomeFlow POSTs the task package JSON to this endpoint: subject / description / outputRequirement / upstreamOutputs / runtimeInputs + callback (completion contract). Return 2xx to accept; when done, POST callback.url with a body like {\"status\": \"success\", \"summary\": \"result summary\"}, authenticated via Authorization: Bearer <callbackToken> (or the 'token' body field). The summary becomes this node's output for downstream tasks.",
      externalRemoteHint: "The task is delegated to a Flow on a remote ClawsomeFlow instance (runs unattended); its leader report becomes this node's output. Pairing: on the remote run csflow external pair-token <name> → locally run csflow external add-remote <name> <secret> → fill that name above. The task text is sent as the remote Flow's runtime prompt; if the remote Flow defines param fields, fill them in the JSON below.",
      externalInputs: "Remote Flow param fields (optional, JSON object)",
      externalInputsHint: "Maps to the remote Flow's param fields: keys are field names, values are the field content, passed as that run's inputs (same as filling the run-params form remotely). Leave empty to send only the task text. Example: {\"requirement\": \"crawl weekly report\", \"target dir\": \"/data/reports\"}",
