...
const en = {
  ...
  flowEditor: {
    ...
    remoteCallInfoCopied:
      "Remote call info copied to clipboard. Paste it into the origin Flow's \"Remote ClawsomeFlow\" subtask (\"Remote Flow call info\" box); saving the subtask registers it automatically.",
    ...
    easyModeNotice:
      "In easy mode, agents automatically bring all their results into your project (including when multiple tasks run in parallel) — no per-result confirmation needed; once everything finishes it still enters the user complaint phase so you can give consolidated feedback. Human checkpoints in the flow are still reviewed by you. A timed run instead finishes directly and skips the complaint phase.",
    easyModeAck: "Got it",
    devMode: "Developer mode",
    devModeSub: "Per-subtask auto-merge control; complaint phase still runs",
    devModeNotice:
      "Developer mode is optimized for software developers, and lets you customize whether each subtask auto-merges changes into the baseline branch. During execution, each agent creates its own unique worktree (an independent branch created from the baseline branch). If you plan to submit PRs directly from worktrees, or need more flexible collaboration control, we recommend disabling agent auto-merge when orchestrating tasks. Agents with auto-merge disabled keep their worktree after the run ends and appear in the \"Pending PRs\" module on the Run detail page (view changes / open a PR in one click / discard the worktree); all other worktrees are deleted automatically after the run ends.",
    devModeAck: "Got it",
    flowName: "Flow name",
    name: "Flow name",
    ...
    validation: {
      ...
      externalRemoteFieldsRequired: "\"{{subject}}\": Remote ClawsomeFlow — paste the \"Remote Flow call info\" (registered automatically when you save the subtask).",
    ...
    taskFields: {
      ...
      externalChannelWebhook: "Generic interface (webhook)",
      ...
      externalWebhookHint: "On dispatch we POST a task package to this endpoint; when done, follow the package’s callback and return {\"status\",\"summary\"}. The summary becomes this node’s output. Absolute paths in upstream summaries may not exist on the partner host — they should not open them locally, and should describe results in plain text.",
      externalRemoteHint: "Delegates to a Flow on a remote ClawsomeFlow; its final report becomes this node’s output. Click \"Copy remote call info\" on the target Flow's editor, paste it above, then save the subtask — the base URL / Flow ID / credential are wired automatically, no CLI needed.",
      externalRemoteHintWithParams: "Delegates to a Flow on a remote ClawsomeFlow; its final report becomes this node’s output. Click \"Copy remote call info\" on the target Flow's editor, paste it above, then save the subtask — the base URL / Flow ID / credential are wired automatically, no CLI needed. The remote Flow declares param fields: upstream tasks are asked to report values for them; below you may optionally fill values you already know (they override the upstream reports).",
  ...
  runDetail: {
    ...
    external: {
      ...
      hint: "Tasks assigned to external executors (human / integrated system / remote ClawsomeFlow). The run continues once the result is submitted.",
      itemTitle: "External task - {{channel}}",
      channelHuman: "Human",
      channelWebhook: "Generic interface (webhook)",
      channelRemoteCsflow: "Remote ClawsomeFlow",
      assignee: "Assignee",
      nodeLabel: "External node",
      showTaskSheet: "Show task sheet",
      summaryPlaceholder: "Completion summary — include absolute paths / links to deliverables. Downstream tasks will see this text.",
      submitSuccess: "Submit as completed",
      submitFailed: "Report failure",
      submitting: "Submitting…",
      waitingWebhook: "Dispatched to the external endpoint — waiting for its callback. You can also submit the result manually above once available.",
