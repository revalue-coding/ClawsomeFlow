#!/usr/bin/env bash
# Runtime gate for real deployment validation on self-hosted server.
...
TEST_RUN_ID="${TEST_RUN_ID:-$(date -u +%Y%m%d%H%M%S)-$("${PYTHON_BIN}" -c 'import secrets; print(secrets.token_hex(3))')}"
...
TEST_PORT="${CSFLOW_TEST_PORT:-27117}"
TEST_BOARD_PORT="${CSFLOW_TEST_BOARD_PORT:-27118}"
TEST_HEALTH_URL="http://127.0.0.1:${TEST_PORT}/"
...
ensure_test_clawteam_runtime() {
  ...
  if [[ -n "${source_spec}" ]]; then
    say "[runtime] install clawteam runtime from source: ${source_spec}"
    ...
    say "[runtime] install clawteam runtime from upstream git"
...
cleanup() {
  ...
  if [[ "${PROD_SERVICE_NAME}" != "${TEST_SERVICE_NAME}" ]]; then
    if [[ "$(systemctl --user is-active "${PROD_SERVICE_NAME}" 2>/dev/null || true)" != "active" ]]; then
      warn "[runtime] production service '${PROD_SERVICE_NAME}' became inactive during cleanup; attempting recovery"
...
export CSFLOW_SERVICE_CPU_QUOTA="${CSFLOW_TEST_CPU_QUOTA:-40%}"
...
export CSFLOW_RUNTIME_TEST_ACTIVE=1
export CSFLOW_RUNTIME_SERVICE_NAME="${TEST_SERVICE_NAME}"
export CSFLOW_RUNTIME_HEALTH_URL="${TEST_HEALTH_URL}"
export CSFLOW_RUNTIME_CLAWTEAM_DATA_DIR="${TEST_CLAWTEAM_DATA_DIR}"
export CSFLOW_RUNTIME_OPENCLAW_HOME="${TEST_OPENCLAW_HOME}"
export CSFLOW_E2E_TEST_ACTIVE=1
export CSFLOW_E2E_BASE_URL="http://127.0.0.1:${TEST_PORT}"
export CSFLOW_PROD_SERVICE_NAME="${PROD_SERVICE_NAME}"
...
say "[runtime] bootstrap isolated data home"
...
say "[runtime] boot isolated csflow test service"
"${TEST_CSFLOW}" start --yes --skip-deps --port "${TEST_PORT}" --board-port "${TEST_BOARD_PORT}"
check_test_health
check_prod_health "after_test_boot"

say "[runtime] run tests/runtime"
"${TEST_PYTHON}" -m pytest -q "${ROOT_DIR}/tests/runtime" -m runtime
