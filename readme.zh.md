...
真实业务总会走出「纯 AI」：有人要检查硬件、审批付款或在模糊处拍板；另一套系统要跑黑盒步骤；另一台机器上已有合适的 Flow。**外部执行节点**把这些执行方放进与 Agent 相同的 DAG——同样的依赖，同样用完成摘要交接。

Flow 编辑：Owner 来源 → **外部执行**，再选 Owner 类型：

| Owner 类型 | 谁来执行 | 结果如何回来 |
|---|---|---|
| **人工** | Run 页上的真人 | 在待办卡片上提交 |
| **远程ClawsomeFlow** | 另一台机器上的某条 Flow | 对端跑完后结果回传 |
| **通用接口（webhook）** | 你自有的 HTTP 服务 | 收任务包 → 完成后回调 |

### 远程ClawsomeFlow（复制粘贴接线，免手敲 CLI）

1. **受理方**：打开目标 Flow 的编辑页，点标题旁的「复制远程调用信息」，得到一段 JSON（含 Flow ID、参数字段、配对凭证），发给发起方。
2. **发起方**：在子任务里选 **远程ClawsomeFlow**，把 JSON 粘进「远端 Flow调用信息」，并**自行填写远端地址**（发起方服务进程能直接访问的主机与端口，例如 `http://192.168.1.10:17017`；勿用 SSH 隧道在浏览器里看到的转发端口），保存子任务即可——Flow ID / 凭证由后台解析注册，凭证只存本机、不写进 Flow。
