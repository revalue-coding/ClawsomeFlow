...
**🧩 Harness 工程** —— 让长周期、多执行方的协作保持稳定，能力变强而流程不散架：拆节点 → 更短上下文、更省 token；人工检查点、子任务局部重跑、任意断点续跑、投诉改进闭环；Worktree 隔离 + **内置跨进程仓库锁**，并行修改不踩坏基线；每次派发/交接/失败都是可回放的 RunEvent；Flow 一次定义、参数化复跑，可跨小时到数月稳定产出。
