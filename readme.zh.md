...
- 想打造一支可在**本地多分支并行开发**的软件工程 Agent 团队：多人设协作、分支隔离推进、最终稳定收敛；
...
|---|---|---|
| 基于 Git worktree 的底层隔离机制，每个 Agent 在独立工作区与分支中并行开发，天然避免串扰与误写；**内置跨进程仓库锁，保证多分支并行开发的绝对可靠性**：无论调度器还是 Agent 发起的合入，都在同一把锁上串行，基线分支并发合入永不损坏 git 元数据；支持智能合入与回滚，并可内置人工检查点，随时进行行为纠正。 | 每一次 dispatch / completion / failure 都记录为 RunEvent——每次运行都可追溯、可回放、可审阅，绝不是黑盒。 | 对结果不满意？发起一次「投诉」，系统会反思、返工，并把经验写回——让下一次比上一次更好。 |
...
> ClawsomeFlow 在此之上，增加了 **AI与精确编排结合、Harness工程、Openclaw/Hermes深度适配、内置跨进程仓库锁（让多分支并行合入绝对可靠——这一点 ClawTeam 完全不可控）、Web 产品化** 等能力。
...
开发者模式为**软件开发协作项目**提供更灵活的协作方式。核心在于:**每个子任务都会收到它所依赖的上游任务的 Agent 与 worktree 信息,并可按任务说明灵活地基于这些信息开展工作**——读取、对比、合入某个分支,或为其提交 PR。

- **每个子任务都拿到上游上下文**:对每个直接依赖,子任务都会被注入上游的 **Agent id、worktree 路径、分支与基线分支**,从而可以灵活地在其成果之上继续——查看、合入该分支、或为其提 PR,完全由你的任务说明驱动。
- **自然语言直接指挥跨分支协作**:在下游任务里只需写一句「把上游 Agent X 的 worktree 分支合入 Y 分支」或「为 X 提交 PR」。Agent 已持有 X 的 worktree/分支/基线信息,外加一段通用「带锁合入」方法,清楚该合什么、怎么合。
- **内置锁 = 并行合入绝对可靠**:每一次合入都通过内置跨进程仓库锁(与调度器同一把锁)串行执行,因此多个分支可以并行开发与合入,永不竞争、永不损坏仓库。ClawTeam 没有仓库级合入锁,无法提供这一保证。
...
- **运行结束自动清理**:Run 结束后,worktree 目录会自动清理。
...
| **并发与隔离** | 并行易竞争，workspace 冲突、上下文串扰 | 解决 OpenClaw 协同不稳定：**多任务并行时 workspace 隔离、可回滚，并彻底解决会话冲突**；**内置跨进程仓库锁，让多分支并行开发与合入绝对可靠** |
...
| **多分支并行可靠性** | **无仓库级合入锁**——基线分支并发合入会竞争、可能损坏 git 元数据，完全不可控 | **内置跨进程仓库锁，保证多分支并行开发的绝对可靠性**——无论调度器还是 Agent，每次合入都在同一把锁上串行，因此你甚至可以用自然语言指挥跨分支合入 / 提 PR 而毫无风险 |
